...
def create_teample():
       if request.method == 'POST':
        ...
        try:
                ...
                elif creator_method == 'student_choice':
                       socketio.emit('creator_method', {'msg': '교수님이 팀 선택 방식으로 팀 생성을 시작했습니다.','class_code': class_code,'mode': creator_method}, room = class_code) 
                       flash(f"소켓 감")        
...
def teample_stu():
        if request.method == 'POST':
                ...
                if current_team_relationship_info: 
                        cursor.execute('SELECT username FROM team_relationship WHERE team_id = ?', (current_team_relationship_info[0][1],))
